...
class WoocommerceBaseExporter(AbstractComponent):
    ...
    def __init__(self, environment):
        ...
        """
        super(WoocommerceBaseExporter, self).__init__(environment)
    ...
    def run(self, binding, *args, **kwargs):
        ...
        self.woocommerce_id = self.binder.to_external(self.binding)
        result = self._run(*args, **kwargs)

        self.binder.bind(self.woocommerce_id, self.binding)
    ...
    def _after_export(self):
        """Create records of dependants woocommerce objects"""
        return


class WoocommerceExporter(AbstractComponent):
    ...
    def __init__(self, environment):
        ...
        :type environment: :py:class:`connector.connector.ConnectorEnvironment`
        """
        super(WoocommerceExporter, self).__init__(environment)
        self.binding = None
    ...
    def _retry_unique_violation(self):
        ...
        record created by :meth:`_export_dependency`), resulting in:
            ...
            constraint "woocommerce_product_template_openerp_uniq"
    ...
    def _get_or_create_binding(
        ...
        relation,
        binding_model,
        binding_field_name="woocommerce_bind_ids",
        bind_values=None,
    ):
        binding = None
        # wrap is typically True if the relation is a 'product.template'
        # record but the binding model is 'woocommerce.product.template'
        ...
        if wrap and hasattr(relation, binding_field_name):
            ...
            if binding:
                ...
                # woocommerce.product.template, it is exported, but we need to
    ...
    def _export_dependency(
        ...
        ``WoocommerceExporter``.  A more precise class can be defined.
        ...
        :param exporter_cls: :py:class:`openerp.addons.connector.\
                             ...
                             By default: WoocommerceExporter
        ...
        :param binding_field_name: name of the one2many towards the bindings
                                   default is 'woocommerce_bind_ids'
    ...
    def _update(self, data):
        ...
        assert self.woocommerce_id
        return self.backend_adapter.write(self.woocommerce_id, data)
    ...
    def _run(self, fields=None, **kwargs):
        ...
        if self.woocommerce_id:
            ...
            # special check on data before export
            self._validate_data(record)
            self.woocommerce_id = self._create(record)
            if self.woocommerce_id == 0:
                raise exceptions.Warning(
                    _("Record on Woocommerce have not been created")
                )

        message = _("Record exported with ID %s on Woocommerce.")
        return message % self.woocommerce_id
